...
export const manageAccounts = onCall(async (request) => {
  ...
  if (callerEmail !== ADMIN_EMAIL || provider !== 'microsoft.com') {
    throw new HttpsError('permission-denied', 'Only the Microsoft administrator account can manage accounts.');
  }
  ...
  if ((action === 'create' || action === 'update') && !email.endsWith(`@${DOMAIN}`)) {
    throw new HttpsError('invalid-argument', `Accounts must use the @${DOMAIN} domain.`);
  }
  if (action === 'create') {
