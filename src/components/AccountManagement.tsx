    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-6">
        ...
        {busy && accounts.length === 0 ? <Loader2 className="h-5 w-5 animate-spin" /> : <div className="divide-y rounded-lg border">
          {accounts.map((account) => <div key={account.email} className="flex items-center justify-between gap-4 p-3"><div><p className="font-medium">{account.email}</p><p className="text-xs text-gray-500">{account.disabled ? 'Disabled' : 'Active'} · Created {new Date(account.createdAt).toLocaleDateString()} · Last login {account.lastLoginAt ? new Date(account.lastLoginAt).toLocaleString() : 'Never'}</p></div><div className="flex gap-1"><Button variant="ghost" size="sm" onClick={() => { setEditing(account); setEmail(account.email); setPassword(''); setDisabled(account.disabled); }}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="sm" className="text-red-600" onClick={() => void remove(account)}><Trash2 className="h-4 w-4" /></Button></div></div>)}
        </div>}
      </CardContent>
    </Card>
  );
