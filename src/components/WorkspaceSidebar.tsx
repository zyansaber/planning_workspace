...
import { ExternalLink, FolderKanban, LayoutGrid, Menu, Monitor, X } from 'lucide-react';
...
export function WorkspaceSidebar({ items, currentId }: WorkspaceSidebarProps) {
  ...
  const renderItem = (item: WorkspaceItem, nested = false) => {
    ...
    const TypeIcon = item.type === 'nested' ? FolderKanban : item.type === 'embed' ? Monitor : ExternalLink;
    ...
    return (
      <button
        ...
        className={cn(
          'group relative flex w-full items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-all duration-200',
      ...
      >
        ...
        <span className={cn(
          ...
          item.color || 'bg-gradient-to-br from-blue-500 to-indigo-600'
        )}>
          <WorkspaceIcon name={item.icon} className="h-5 w-5" />
        </span>
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-0.5 flex items-center gap-1 text-[11px] text-slate-400">
            ...
            {item.type === 'nested' ? 'Collection' : item.type === 'embed' ? 'Workspace' : 'External link'}
          </span>
        </span>
      </button>
    );
