...
Before saving the first password account, deploy the included Realtime Database
rules to the `planningworkspace` project:

```bash
firebase login
firebase use planningworkspace
firebase deploy --only database
```

Without these rules, Firebase returns `PERMISSION_DENIED` and the Settings form
cannot create the `passwordUsers` node. The included rules allow the Microsoft
administrator to list/write password accounts, allow login to read only a known
account key, and keep other database paths closed except read-only workspaces.
